...
| Data Ingestion Service | データ取得・キャッシュ管理。ティッカー単位で最新取得時刻を監視し、**主経路はDukascopy HTTPバースト（遅延中央値6秒/p95 12秒）**、**短期フォールバックはyfinance（中央値24秒/p95 58秒）**とする。`provider=yfinance`でp99>120秒または連続3失敗、`provider=dukascopy`でp99>25秒または連続3失敗を検知した場合は`HealthState`へ`data_latency`理由を通知し、`Spread Monitor`/`Emergency Orchestrator`へ遅延イベントを伝播する。フォールバックはDukascopy→ローカルキャッシュ→手動CSVの順で切替え、Runbook `docs/runbooks/RUN-DATA-05.md`（AC-45）および`docs/runbooks/RUN-DATA-06.md`（AC-04/AC-45）で訓練済みのエスカレーションを実施する。**公式API移行計画**として、M1.2でRefinitiv Elektron Lite / dxFeed FX / OANDA premium APIを比較し（費用≦月額¥80,000・ライセンスで個人利用許諾・REST/WebSocketで再配信不可を遵守）、PoCでp95≤12秒を満たしたプロバイダを`Data Ingestion Adapter`へ追加する。導入条件はコンプライアンス承認とSLA 30日ローリング合格、フェイルオーバー統合テスト（AC-45拡張）を満たすこと。 | yfinance, Dukascopy, CSV（M1）、有償API（M1.2+オプション） | M1 Core |
...
- **実装判断指針**:
  - **主経路優先順位**: ライブ取得はDukascopy→yfinance→ローカルキャッシュ→手動CSVの順とし、yfinanceはYahoo利用規約が想定する個人利用・非再配信の範囲に限定する。Dukascopy取得はHTTPバーストを基本とし、1分に1回のキャッシュ書き込み＋最大同時3ペアまでの取得に制限して帯域とレート制限を回避する。
...
| soft_stop | オペレータ確認後、復帰コマンド | ok | `tradectl health resume`で復帰。Kill Switchの手動解除はRunbook `docs/runbooks/RUN-RISK-01.md`（AC-03/AC-09）に従う。
