...
- **実装判断指針**:
  ...
  - **フェイルオーバー閾値**: `provider=yfinance`で`latency_p99>120`または連続3失敗を検出したら即座にDukascopyへ切替え、Dukascopyでも`latency_p99>25`が継続する場合はローカルキャッシュ→手動CSV（Runbook `RUN-DATA-05`）へ移行する。切替履歴は`reports/performance/data_latency/<YYYYMMDD>.md`と`logs/events/data_latency.jsonl`へ同時記録し、Validation Data Playbook（AC-45）でレビューする。
...
- **遅延伝播シーケンス**: `warning`レベルの遅延はEvent Bus経由で`Spread Monitor`へ配信され、対象シンボルの`SpreadCooldownState`を`halt`へ強制遷移し、`tradectl status`/メールへアラートを送出する。`critical`レベル（>60秒または連続失敗）はM1ではKill SwitchソフトストップとRunbook `RUN-DATA-05`による手動承認フローのみを起動し、M2+で`Emergency Orchestrator`が`emergency.yaml`の`data_latency`プレイブックを評価してCLI/メール通知と自動シナリオ実行を追加する。復旧後は`HealthMonitor.ack`で解除し、Spread Monitorを通常状態へロールバックする。
