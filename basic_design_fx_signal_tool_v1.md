...
### 1.1 マイルストーン適用範囲
- **M1 Core (M1.0)**: FR-01〜FR-08, FR-10, FR-18にフォーカスし、データ取得/品質監視、MA+RSIベースのシグナル生成、Kill Switch・リスク制御、HITLチケット（承認/却下/編集）、Resync & Snapshot、週次レポート（**M1 Coreテンプレート: Sharpe/最大DD/WinRate/累積Rのみ**）を実装。監査/署名/高度ガバナンスは敢えて外し、Paper 90営業日運用とオペレーションスループット測定に集中する。Data Ingestionは**常時4ペア並列取得と60分以内のCatch-up完了**をKPI化し、SLAターゲット自体は**計測と手動フェイルオーバードリルの確立**を完了条件とする（Runbook `RUN-DATA-05/06`）。実測が閾値未達の場合は`HealthState=degraded(reason=data_latency_fetch)`または`soft_stop(reason=data_latency_processing)`でAcceptable Degradation運用（代替ソース＋手動CSVで主要4ペア継続）へ移行し、復旧計画を7日以内に提示する。Alpha Scoreboard/Idea Pipeline等のM2ガバナンス機構はスタブ化してFeature Flagを既定OFFで維持し初期リリースの複雑化を避ける。
...
- **遅延伝播シーケンス**: `warning(fetch)`はEvent Bus経由で`Spread Monitor`へ配信し、対象シンボルの`SpreadCooldownState`を`halt`へ強制遷移。`warning(processing)`は`FeaturePipeline`の`bar_ready_queue`深さを監視し、処理遅延のみでKill SwitchがSTOPに遷移しないよう`soft_stop(processing)`までに抑える。`critical(fetch)`（`fetch_timeout`連続3回）はKill SwitchソフトストップとRunbook `docs/runbooks/RUN-DATA-05.md`を起動し、`critical(processing)`は`ManualCsvIngestionTask`完了まで`Workflow`をディレイキューに切替える。M2+では`Emergency Orchestrator`が`data_latency`プレイブックを評価し、CLI/メール通知と自動シナリオ実行を追加する。復旧後は`HealthMonitor.ack`で解除し、Spread Monitorと`bar_ready_queue`のバックログを通常状態へロールバックする。
...
- **Health Monitor**: SPRT評価（FR-22）、Kill Switch（FR-05）、連続エラー検出（FR-12）、SLAロールアップ（`SlaMinuteRollup`）を統合して`HealthState`を更新し、Signal Engine/Mode Controllerへブロードキャスト。`HealthState`は`{"status":"ok|soft_stop|hard_stop","reasons":[...]}`形式で保持し、SLA関連では`reasons[].code ∈ {data_latency_fetch, data_latency_processing, data_down, manual_stop, planned_maintenance}`を付与する。CLIの`tradectl health show`で確認し、`tradectl health manual-stop --reason <msg>`は`manual_stop=true`を、`tradectl health resume`は`manual_stop=false`と`manual_stop_released`イベントを残してSLA分母から除外・復帰を制御する。状態遷移と運用アクションは以下。
...
| ok | fetch遅延SLA未達（fetch_delay_p95/p99超過が連続3回、フェイルオーバー発火） | soft_stop | `HealthState=degraded(reason=data_latency_fetch)`へ遷移し、Dukascopy→キャッシュ→手動CSVの順で取得を継続。`reports/governance/provider_switch_checklist.md`に代替計画（再ETL/プロバイダ切替）と7日以内の実施期限を記録し、月次SLAレビューで承認されるまで`soft_stop`を維持する。
| ok | processing遅延SLA未達（processing_delay_p95/p99超過が連続3回） | soft_stop | `HealthState=soft_stop(reason=data_latency_processing)`へ遷移し、`ManualCsvIngestionTask`完了と`processing_delay`改善を待つ。`reports/validation_log/AC-45_sla_<date>.md`に対応メモを残し、Quality Guard改善が確認できるまで`bar_ready_queue`を監視する。
...
- **検証データ支援スクリプト**: `make data-build`でDukascopy→Parquet変換、`make sla-report`で`metrics/pipeline_latency.jsonl`と`metrics/data_ingestion_sla.jsonl`（`phase=fetch|processing`）のSLA集計を実行し、取得遅延と処理遅延を分離したレポートを`reports/validation_log/AC-45_sla_<date>.md`へ出力する。CIでは`make validation-check`を追加し、Validation Data Playbookのサインオフとハッシュ整合を自動検査する。
