...
- **Board Guard Policy**: `HealthState.status`とは別に`board_mode ∈ {normal, guarded, halted}`を追跡する。`guarded`はAcceptable Degradation運用時に自動設定され、新規シグナル生成を停止したうえでReduce-Only提案と復旧チェックリスト操作（`RUN-DATA-05/06`の手順）だけを許可する。`board_mode=guarded`は「データ鮮度検証→Reduce-Only運用→復旧確認→提案再開」のチェックリスト完了と`degraded_ack`監査イベント発行を条件に`ResyncCompleted`または`health.resume(reason=data_latency_recovered)`で解除し、Kill Switchによる`soft_stop/hard_stop`とは独立して管理する。`halted`はKill Switch `soft_stop`/`hard_stop`でのみ使用する。状態遷移と運用アクションは以下。
...
| ok | fetch遅延SLA未達（fetch_delay_p95/p99超過が連続3回、フェイルオーバー発火） | board_guarded | `HealthState=degraded(reason=data_latency_fetch)`へ遷移し、Dukascopy→キャッシュ→手動CSVの順で取得を継続。Signal Boardは`BoardMode=guarded`で新規提案を停止し、Reduce-Only運用と復旧チェックリスト入力のみ許可する。`reports/governance/provider_switch_checklist.md`に代替計画（再ETL/プロバイダ切替）と7日以内の実施期限を記録し、解除はSLA回復＋PO/Opsダブルサイン＋`degraded_ack`発行で実施。
