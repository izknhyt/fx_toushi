...
- **Health Monitor**: SPRT評価（FR-22）、Kill Switch関連メトリクス（FR-05）、連続エラー検出（FR-12）、SLAロールアップ（`SlaMinuteRollup`）を統合して`HealthState`の計測値と推奨アクションを生成する。`HealthState`は`{"status":"ok|soft_stop|hard_stop","reasons":[{"code":...,"detail":...,"recommended_action":...}]}`形式で保持し、SLA関連では`reasons[].code ∈ {data_latency_fetch, data_latency_processing, data_down, manual_stop, planned_maintenance, rate_limited}`を付与する。CLIの`tradectl health show`で確認し、Runbook `RUN-DATA-05/06`に沿ってオペレータが`tradectl board --guarded`, `tradectl health manual-stop`, `tradectl kill-switch engage --reason <code>`などの手動操作を判断する。閾値超過時は`health.changed`イベントへ`recommended_action`（例: `runbook:RUN-DATA-05#enter_guarded`, `notify:ops`, `sla_report:append`）と計測根拠（`metric`, `observed_value`, `threshold`)を添付し、OpsチームがRunbookで承認・記録する。`degraded`状態が**連続3営業日**または**ローリング30日で2回**発生した場合は`health.escalate`イベントを発行してレビュー会議を要求するが、Kill Switch/Board Guardの切替はRunbookレビュー後に人間が承認する。`degraded`が**5営業日超継続**または週次レビュー2回連続で解消しない場合は`severity=critical`で通知し、Ops ManagerがRunbook `RUN-DATA-05`に基づきKill Switch `hard_stop`発動の是非を手動判断し、承認サインを`reports/validation_log/AC-45_sla_<date>.md`へ記録する。`catch_up_lag_minutes`が警告閾値を超えた際は`notify:ops`/`pager:ingestion`推奨と共に`reports/validation_log/AC-45_sla_<date>.md`へ必須記録フィールド（測定値・代替ソース実施状況・承認者）を追加するよう指示し、復帰時もRunbook手順で`catch_up_lag_minutes<30`、SLAメトリクス回復、手動CSV二重入力ハッシュ一致を確認した上で`tradectl health resume`と`degraded_ack`監査イベントを人手で発行する。

- **Board Guard Policy**: `HealthState.status`とは別に`board_mode ∈ {normal, guarded, halted}`を追跡する。`board_mode`の遷移はRunbook承認済みの手動操作のみで行い、`tradectl board --guarded`/`tradectl board --normal`/`tradectl board --halted`コマンドの実行ログと承認者サインを`docs/runbooks/RUN-DATA-05/06`へ記録する。Workflow Orchestratorは`board_mode_changed`イベントを受信するとSignal EngineとRisk Managerへブロードキャストし、Signal Engineは次バー評価前に`board_mode`を参照して`SignalAction=block`へ切り替える。既存シグナルがTTL内で残っている場合は`board_guard.blocked`監査イベントを追記し、リプレイ復旧時にも同シーケンスを再実行して重複停止を防ぐ。Reduce-Only提案のみ許可するフェーズではWorkflow Orchestratorが`board_mode=guarded`の間、Signal Engineへ`allow_reduce_only=true`フラグを付加した制御メッセージを渡し、Signal Engineは候補生成後のフィルタ処理でReduce-Only以外のアクションを一律破棄する。`board_mode=guarded`は「データ鮮度検証→Reduce-Only運用→復旧確認→提案再開」のチェックリスト完了と`degraded_ack`監査イベント発行を条件に解除し、Kill Switchによる`soft_stop/hard_stop`とは独立して管理する。`halted`はKill Switchコマンド実行時にのみ使用する。状態遷移と運用アクションは以下。

| 現状態 | トリガー | 次状態 | オペレータ対応 |
| --- | --- | --- | --- |
| ok | Runbook `RUN-RISK-01`に基づくKill Switch判断（daily_loss/weekly_loss/その他レビュー結果） | soft_stop | Ops/POが `tradectl kill-switch engage --reason <code>`を実行し、`tradectl status`で原因確認。解除時はRunbookチェックを完了させ`tradectl kill-switch release --ticket <id>`→`tradectl health resume`の順で復帰。
| ok | Spread guardチェックリストで閾値超過を確認 | soft_stop | `tradectl spread cooldown`で解除条件を監視しつつRunbook `RUN-RISK-02`でReduce-Only運用へ切替え。解除は責任者承認後に`tradectl health resume`。
| ok | Runbook `RUN-DATA-05`でデータ欠損を確認 | soft_stop | `tradectl health manual-stop --reason data_gap`で停止し、`tradectl resync`/手動CSV投入を実施。正常バー3本確認後に承認者サインを取得し`tradectl health resume`で復帰。
| ok | SLA遅延レビューでGuarded運用が必要と判断 | board_guarded | `tradectl board --guarded --reason <code>`を実行し、Workflow Orchestrator経由でSignal Engineが`SignalAction=block`へ切替え。Runbook `RUN-DATA-05/06`に沿ってDukascopy→キャッシュ→手動CSVの順で取得を継続し、`reports/governance/provider_switch_checklist.md`へ代替計画と期限を記録。SLA回復＋PO/Opsダブルサイン＋`degraded_ack`発行で`tradectl board --normal`を実行。
| ok | processing遅延/バッチ詰まりが継続し手動介入が必要 | board_guarded | `tradectl board --guarded`でReduce-Onlyモードへ切替え、`ManualCsvIngestionTask`や再処理を手動で実施。`reports/validation_log/AC-45_sla_<date>.md`に対応メモとレビュー結果を残し、`bar_ready_queue`安定後に`tradectl board --normal`。
| soft_stop | Runbook完了後の復帰判断 | ok | 承認サインが揃い次第`tradectl health resume`。Kill Switch解除時は`docs/runbooks/RUN-RISK-01.md`のチェックリスト通りに実施。
| 任意 | `tradectl stop`/manual emergency、クリティカル例外 | hard_stop | プロセス停止。再起動前に原因調査し、Runbook `RUN-OPS-01`で承認後に`tradectl start`。
| hard_stop | 再起動完了 | ok | スナップショットから復旧後、`tradectl status`で整合性確認しRunbookサインを取得。
...
- **シグナル評価**: 再計算したバーについてRegime/Scoring/Execution/Risk/Sizingを再実行し、TTL切れやドリフト超過のチケットは自動失効マーク。Acceptable Degradation中は`BoardMode=guarded`で新規提案を停止し、Reduce-Only運用と復旧チェックリスト完了後に`ResyncCompleted`イベントを契機として通常モードへ自動復帰させる。
