...
### 1.1 マイルストーン適用範囲
- **M1 Core (M1.0)**: FR-01〜FR-08, FR-10, FR-18にフォーカスし、データ取得/品質監視、MA+RSIベースのシグナル生成、Kill Switch・リスク制御、HITLチケット（承認/却下/編集）、Resync & Snapshot、週次レポート（**M1 Coreテンプレート: Sharpe/最大DD/WinRate/累積Rのみ**）を実装。監査/署名/高度ガバナンスは敢えて外し、Paper 90営業日運用とオペレーションスループット測定に集中する。Data Ingestionは**常時4ペア並列取得と60分以内のCatch-up完了**をKPI化し、SLAターゲット自体は**計測と手動フェイルオーバードリルの確立**を完了条件とする（Runbook `RUN-DATA-05/06`）。実測が閾値未達の場合は`HealthState=degraded(reason=data_latency)`でAcceptable Degradation運用（代替ソース＋手動CSVで主要4ペア継続）へ移行し、復旧計画を7日以内に提示する。Alpha Scoreboard/Idea Pipeline等のM2ガバナンス機構はスタブ化してFeature Flagを既定OFFで維持し初期リリースの複雑化を避ける。
...
| Data Ingestion Service | データ取得・キャッシュ管理。ティッカー単位で最新取得時刻を監視し、**主経路はDukascopy HTTPバースト（遅延中央値6秒/p95 12秒）**、**短期フォールバックはyfinance（中央値24秒/p95 58秒）**とする。`asyncio`ベースのワーカーキューで**常時4ジョブ（4ペア）並列取得**を保証し、Catch-upモードでは`max_concurrency=6`までスケールする。`metrics/data_ingestion_sla.jsonl`へ同時実行数と遅延を記録し、30分内に4ペア全履歴を追いつけなければ`health.changed(reason=data_latency)`を発火する。`provider=yfinance`でp99>120秒または連続3失敗、`provider=dukascopy`でp99>25秒または連続3失敗を検知した場合は`HealthState`へ`data_latency`理由を通知し、`Spread Monitor`/`Emergency Orchestrator`へ遅延イベントを伝播する。フォールバックはDukascopy→ローカルキャッシュ→手動CSVの順で切替え、Runbook `docs/runbooks/RUN-DATA-05.md`（AC-45）および`docs/runbooks/RUN-DATA-06.md`（AC-04/AC-45）で訓練済みのエスカレーションを実施し、yfinanceへの恒常依存を避ける。Acceptable Degradation期間中は`HealthState=degraded(reason=data_latency)`で新規シグナルを`soft_stop`に留めつつ、手動CSV追記と`reports/governance/provider_switch_checklist.md`で代替計画を7日以内に提示する。安定復帰後は`stabilization_elapsed`を記録し、未達指標はValidation Data Playbook月次レビューで解決策をトラッキングする。**公式API移行計画**として、M1.2でRefinitiv Elektron Lite / dxFeed FX / OANDA premium APIを比較し（費用≦月額¥80,000・ライセンスで個人利用許諾・REST/WebSocketで再配信不可を遵守）、PoCでp95≤12秒を満たしたプロバイダを`Data Ingestion Adapter`へ追加する。導入条件はコンプライアンス承認とSLA 30日ローリング合格、フェイルオーバー統合テスト（AC-45拡張）を満たすこと。 | yfinance, Dukascopy, CSV（M1）、有償API（M1.2+オプション） | M1 Core |
...
- **実装判断指針**:
  - **主経路優先順位**: ライブ取得はDukascopy→yfinance→ローカルキャッシュ→手動CSVの順とし、yfinanceはYahoo利用規約が想定する個人利用・非再配信の範囲に限定する。Dukascopy取得はHTTPバーストを基本とし、`asyncio`ワーカーで**常時4ペア並列（必須ティッカー4種）**を稼働させ、Catch-up時は`max_concurrency=6`まで拡張する。各ワーカーは1分ごとにキャッシュへ書き込み、スロット枯渇時は`priority=critical`の欠損補完ジョブが最優先で割り込む。
  - **フェイルオーバー閾値**: `provider=yfinance`で`latency_p99>120`または連続3失敗を検出したら即座にDukascopyへ切替え、Dukascopyでも`latency_p99>25`が継続する場合はローカルキャッシュ→手動CSV（Runbook `docs/runbooks/RUN-DATA-05.md` → `docs/runbooks/RUN-DATA-06.md`）へ移行する。切替履歴は`reports/performance/data_latency/<YYYYMMDD>.md`と`logs/events/data_latency.jsonl`へ同時記録し、Validation Data Playbook（AC-45）でレビューする。
...
- **ヘルスチェック/SLA監視**: ティッカー毎に最新バー取得時刻を`symbol_last_ts`として保持し、`latency_sec = (now_utc - symbol_last_ts).total_seconds()`で遅延を算出。直近30日ローリングで`latency_p95`/`latency_p99`と`success_rate`を集計し、**yfinance: p95≤60秒・p99≤120秒・成功率≥97.5%／Dukascopy: p95≤15秒・p99≤25秒・成功率≥99.0%**をターゲットとする。`latency_p95`/`latency_p99`が各閾値を超過、または`success_rate`が目標を下回った場合は`HealthMonitor.raise(level='warning', reason='data_latency', provider=...)`を送出し、`latency_sec>60`または連続3回失敗で`level='critical'`を発火してKill Switchソフトストップを準備する。1分ごとに`SlaMinuteRollup`が`ops_schedule.yaml`と`maintenance_windows.yaml`を参照して`status ∈ {up, down, excluded}`を判定し、`provider=yfinance`は`latency_sec≤120`かつ連続失敗<3、`provider=dukascopy`は`latency_sec≤25`かつ連続失敗<3かつ`manual_stop`フラグが立っていない場合に`up`、それ以外を`down`、`manual_stop`/`planned_maintenance`中を`excluded`として`metrics/data_ingestion_sla.jsonl`へ追記する。分単位集計は`uptime_minutes`/`downtime_minutes`/`excluded_minutes`と稼働率（プロバイダ別）をJSON（`{window_start, window_end, provider, uptime_ratio}`）に出力し、`tradectl data health`でSLA逸脱理由を可視化する。
...
- **遅延伝播シーケンス**: `warning`レベルの遅延はEvent Bus経由で`Spread Monitor`へ配信され、対象シンボルの`SpreadCooldownState`を`halt`へ強制遷移し、`tradectl status`/メールへアラートを送出する。`critical`レベル（>60秒または連続失敗）はM1ではKill SwitchソフトストップとRunbook `docs/runbooks/RUN-DATA-05.md`/`docs/runbooks/RUN-DATA-06.md`による手動承認フローのみを起動し、M2+で`Emergency Orchestrator`が`emergency.yaml`の`data_latency`プレイブックを評価してCLI/メール通知と自動シナリオ実行を追加する。復旧後は`HealthMonitor.ack`で解除し、Spread Monitorを通常状態へロールバックする。
- **フェイルオーバー手順/ログ**: 1) `latency_sec>60`または`HealthMonitor`の`critical`シグナルでDukascopy高速フェッチへ自動切替（`failover_stage=primary->secondary`）。2) Dukascopyも不可の場合はローカルキャッシュ`data/raw/<provider>/<symbol>/<tf>.parquet`を読み出してギャップを埋め、`cache_source=fallback`をマーキングし、`tradectl data failover --to cache`で明示化。3) それでも欠損が残る場合はRunbook `docs/runbooks/RUN-DATA-06.md`に従い手動CSV（`data/manual/<date>/<symbol>.csv`）を投入し、`tradectl data reload --source manual`で取り込む。4) フェイルオーバー実行ごとに`failover_trigger_ts`/`failover_reason`/`stabilization_elapsed`/`sla_status_before`をJSONLへ記録し、復旧後5分間安定性を監視してから`tradectl data resume --provider <name>`で主経路へ復帰する。手動モード中は`manual_source=true`フラグをイベントに添付し、ライセンス制約（配信範囲、商用可否）チェックリストの実施結果を`reports/audit/license/`へ保存する。
...
- **Health Monitor**: SPRT評価（FR-22）、Kill Switch（FR-05）、連続エラー検出（FR-12）、SLAロールアップ（`SlaMinuteRollup`）を統合して`HealthState`を更新し、Signal Engine/Mode Controllerへブロードキャスト。`HealthState`は`{"status":"ok|soft_stop|hard_stop","reasons":[...]}`形式で保持し、SLA関連では`reasons[].code ∈ {data_latency, data_down, manual_stop, planned_maintenance}`を付与する。CLIの`tradectl health show`で確認し、`tradectl health manual-stop --reason <msg>`は`manual_stop=true`を、`tradectl health resume`は`manual_stop=false`と`manual_stop_released`イベントを残してSLA分母から除外・復帰を制御する。状態遷移と運用アクションは以下。
...
| ok | SLA未達（遅延p95/p99超過が連続3回、フェイルオーバー発火） | soft_stop | `HealthState=degraded(reason=data_latency)`へ遷移し、Dukascopy→キャッシュ→手動CSVの順で取得を継続。`reports/governance/provider_switch_checklist.md`に代替計画（再ETL/プロバイダ切替）と7日以内の実施期限を記録し、月次SLAレビューで承認されるまで`soft_stop`を維持する。
