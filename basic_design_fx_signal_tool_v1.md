...
- **プロバイダ別更新頻度/再接続ポリシー**: yfinanceは`config/sla_thresholds/<profile>.yaml::rate_limit.poll_stages`で定義したStage（0:平均15秒`Uniform[12,15]`, 1:平均13秒`Uniform[11,14]`, 2:平均11秒`Uniform[10,12]`）と`rate_limit.tokens_per_minute`を読み込み、`RateLimitGuard`のトークンバケット（`burst_tokens`含む）でポーリング間隔と同時実行数を制御する。Stage遷移要件は**429発生率≤1.0%（Rolling 1h）かつ連続429<3回を7日継続**で昇格、**429発生率>1.5%または連続429≥3回**で直前Stageへロールバックとし、ロールバック時は即座に`ProviderFetchWorker`のスロット上限を引き下げる。`RetryPolicy(max_attempts=3, backoff=1.5)`は各Stage共通で適用し、連続失敗時は`cooldown_sec=60`でリトライ抑制。DukascopyはHTTPバースト取得を**基準4秒周期（Stage連動で最大3秒まで短縮）**で行い、失敗時は`retry<=5`の指数バックオフ（最大間隔20秒）を実施する。WebSocketモード（M2+）は`ping_interval=15s`、`pong_timeout=5s`で監視し、`tradectl data reconnect --provider dukascopy`で強制リセット可能。手動CSVモードは`manual_source=true`で健康監視を除外するが、`tradectl data resume --provider <name>`で既定経路に戻す。
  - **遅延伝播シーケンス**: `warning(fetch)`はEvent Bus経由で`Spread Monitor`へ配信し、対象シンボルの`SpreadCooldownState`を`halt`へ強制遷移した上で`BoardGuardPolicy.activate(symbols=core_pairs)`を発火し、新規シグナル生成を停止した`BoardMode=guarded`へ移行する。`warning(processing)`は`FeaturePipeline`の`bar_ready_queue`深さを監視し、処理遅延のみでKill SwitchがSTOPに遷移しないようReduce-Only操作と復旧チェックリスト記録のみを許可する。`critical(fetch)`（`fetch_timeout`連続3回）は手動CSVフェイルオーバーとRunbook `docs/runbooks/RUN-DATA-05.md`の実施を要求し、Kill Switchは維持したまま`BoardMode=guarded(core_pairs_only=true)`で新規提案ロックとReduce-Only限定運用を続ける。`critical(processing)`は`ManualCsvIngestionTask`完了まで`Workflow`をディレイキューに切替えるが、既存ポジションのReduce-Only操作とチェックリスト進行ログの更新は継続可能とする。M2+では`Emergency Orchestrator`が`data_latency`プレイブックを評価し、CLI/メール通知と自動シナリオ実行を追加する。復旧後は`HealthMonitor.ack`で解除し、Spread Monitor・`bar_ready_queue`・`BoardMode`を通常状態へロールバックする。
...
- **Board Guard Policy**: `HealthState.status`とは別に`board_mode ∈ {normal, guarded, halted}`を追跡する。`guarded`はAcceptable Degradation運用時に自動設定され、新規シグナル生成を停止したうえでReduce-Only提案と復旧チェックリスト操作（`RUN-DATA-05/06`の手順）だけを許可する。Workflow Orchestratorは`HealthMonitor`からの`board_mode_changed`イベントを受信すると、Signal EngineとRisk Managerへブロードキャストし、Signal Engineは次バー評価前に`board_mode`を参照して`SignalAction=block`へ切り替える。既存シグナルがTTL内で残っている場合は`board_guard.blocked`監査イベントを追記し、リプレイ復旧時にも同シーケンスを再実行して重複停止を防ぐ。Reduce-Only提案のみ許可するフェーズではWorkflow Orchestratorが`board_mode=guarded`の間、Signal Engineへ`allow_reduce_only=true`フラグを付加した制御メッセージを渡し、Signal Engineは候補生成後のフィルタ処理でReduce-Only以外のアクションを一律破棄する。`board_mode=guarded`は「データ鮮度検証→Reduce-Only運用→復旧確認→提案再開」のチェックリスト完了と`degraded_ack`監査イベント発行を条件に`ResyncCompleted`または`health.resume(reason=data_latency_recovered)`で解除し、Kill Switchによる`soft_stop/hard_stop`とは独立して管理する。`halted`はKill Switch `soft_stop`/`hard_stop`でのみ使用する。状態遷移と運用アクションは以下。
...
| ok | fetch遅延SLA未達（fetch_delay_p95/p99超過が連続3回、フェイルオーバー発火） | board_guarded | `HealthState=degraded(reason=data_latency_fetch)`へ遷移し、Dukascopy→キャッシュ→手動CSVの順で取得を継続。Signal Boardは`BoardMode=guarded`で新規提案を停止し、Workflow OrchestratorがSignal Engineへ`SignalAction=block`命令を配信して新規キュー投入を停止、Risk ManagerはReduce-Only提案以外を拒否する。`reports/governance/provider_switch_checklist.md`に代替計画（再ETL/プロバイダ切替）と7日以内の実施期限を記録し、解除はSLA回復＋PO/Opsダブルサイン＋`degraded_ack`発行で実施。
| ok | processing遅延SLA未達（processing_delay_p95/p99超過が連続3回） | board_guarded | `HealthState=degraded(reason=data_latency_processing)`へ遷移し、`ManualCsvIngestionTask`完了と`processing_delay`改善を待つ。Signal Boardは`BoardMode=guarded`でReduce-Only操作とRunbookチェックリスト更新のみを許可し、Workflow Orchestrator経由の`SignalAction=block`でSignal Engineがキュー投入を停止、Risk ManagerはReduce-Only以外を拒否する。`reports/validation_log/AC-45_sla_<date>.md`に対応メモと手動レビュー結果を残す。`bar_ready_queue`が安定するまで`degraded_ack`を発行しない。
...
11. Signal Engineが戦略プラグインを順に評価し、候補シグナルを生成。`m1_baseline_ma_rsi`プラグインは5分足EMA(21/55)クロス＋RSIゾーンの条件に加えて、1時間足EMA(55)の傾きがエントリー方向と一致した場合のみ`RawSignal`を返却し、日足Zスコアは`badges`に反映される。〔M1〕[^ms-core]
    - `board_mode=guarded`の間はWorkflow Orchestratorからの`SignalAction=block`指示に従ってSignal Engineが候補シグナルをキューへ投入せず、Reduce-Only提案フラグが付与された既存候補だけをMaintainする。Risk Managerも同モード中はReduce-Only属性を持たない要求を`risk.decision=reject(board_guard)`で即時却下する。〔M1〕
