...
### 1.1 マイルストーン適用範囲
- **M1 Core (M1.0)**: FR-01〜FR-08, FR-10, FR-18にフォーカスし、データ取得/品質監視、MA+RSIベースのシグナル生成、Kill Switch・リスク制御、HITLチケット（承認/却下/編集）、Resync & Snapshot、週次レポート（**M1 Coreテンプレート: Sharpe/最大DD/WinRate/累積Rのみ**）を実装。監査/署名/高度ガバナンスは敢えて外し、Paper 90営業日運用とオペレーションスループット測定に集中する。Data Ingestionは**常時4ペア並列取得と30分以内のCatch-up完了**をKPI化し、SLAターゲット自体は**計測・逸脱検知・手動フェイルオーバードリルの確立**を完了条件とする（Runbook `RUN-DATA-05/06`）。`HealthMonitor`はp95/p99遅延や429/403レート制限イベントを計測し、`metrics/data_ingestion_sla.jsonl`へ**推奨アクション（`suggest_board_mode=guarded`, `suggest_failover=dukascopy`等）付きで記録するだけに留める**。実測が閾値未達の場合はOpsチームがRunbookに沿って`HealthState=degraded(reason=data_latency_fetch|processing)`を宣言し、`tradectl board --guarded`や`tradectl data failover --mode manual`/Kill Switch操作を手動で行う。Kill Switchが`STOP`へ遷移した際はRunbook `RUN-EMER-UNWIND-01`で`tradectl emergency close-all/cancel-open/hedge`フローと証跡収集を完遂し、Ops/Risk/POダブルサイン後にのみ解除審査へ進む。`BoardMode=guarded`中はReduce-Only提案と復旧チェックリスト操作のみを許可し、解除時に`degraded_ack`イベントとRunbookチェックリストのサインを必須化する。復旧計画は7日以内に提示し、`catch_up_lag_minutes<30`、SLAターゲット復帰、手動CSV二重入力ハッシュ一致とPO/Opsサインを条件に平常モードへ戻す。`degraded`が**連続3営業日またはローリング30日で2回**発生した場合はOps Manager主導でエスカレーションレビューを開き、**5営業日超継続または週次レビュー2回未解消**の際はKill Switch `hard_stop`発動を手動判断する。週次レポートとKPI出力では`metric_state ∈ {confirmed, provisional, pending}`を明示し、Paper初期90営業日は暫定値＋手動レビューコメントのみを必須とする。Alpha Scoreboard/Idea Pipeline等のM2ガバナンス機構はスタブ化してFeature Flagを既定OFFで維持し初期リリースの複雑化を避ける。Runbook依存の運用完了判定は要件定義[§10.1 Runbookインベントリ](要件定義（テンプレ形式）v_1.md#runbook-inventory)で`status=ready`を満たすことを条件とし、`RUN-DATA-05/06`レビューの最新サインオフが欠ける場合はOps Readiness Evaluatorがリリースゲートをブロックする。
- **M1.1 Hardening**: FR-09, FR-11〜FR-18, FR-23, FR-27, FR-30, FR-38, FR-44を追加し、監査ログ冗長化、監査パック生成/リリースゲート、設定ガバナンス、回帰テスト、Paper-Liveパリティ、ニュースガード/Spread Cooldown、Validation Data Playbookによるデータ整備、ガードレールメトリクス整合、スナップショット/バックアップ強化を完了する。AC-08およびAC-12〜AC-45をカバーし、Live少額トライアルのゲートを準備する。
...
| Session Manager | アプリ全体の起動・終了、Catch-up調整。スナップショットと最新イベントの差分から欠損ウィンドウを検出し、`resync_queue`へ`priority={critical,high,normal}`付き`BackfillJob`を投入する。起動前にRunbook `RUN-TIME-01`で時計差を手動確認し、ズレがある場合はオペレータ判断で`board_mode=guarded`を選択する。`TimeSyncGuard`による自動測定と強制ガードはM1.1 Hardeningでの追加項目として扱い、M1 Coreは警告ログ出力までに留める。長時間停止（24h超）では主要4ペアに`priority=critical`を付与し、復旧完了までは`HealthState=soft_stop(manual_review)`を維持する。 | Python service | M1 Core |
| Mode Controller | Backtest/Paper/Liveの振る舞い切替 | Stateパターン | M1 Core |
| Data Ingestion Service | データ取得・キャッシュ管理。ティッカー×プロバイダ単位で**`asyncio.Queue(maxsize>1)`を持つフェッチワーカー**と**内部バッファ**を用意し、取得（fetch）と整形/検証（processing）を分離する。主経路はDukascopy（中央値6秒/p95 12秒）、短期フォールバックはyfinance（中央値24秒/p95 58秒）。`ProviderFetchWorker`が常時4ジョブ並列、Catch-up時は6ジョブまで拡張し、`ProviderParseWorker`が`AsyncBuffer`内でUTC整列・Quality Guard検証を完了させてから`bar_ready_queue`へ整合済みバーのみ投入する。`FallbackRetryTask`が再試行間隔を管理し、手動CSV移行は`ManualCsvIngestionTask`へ委譲してパイプライン本流から切り離す。`ManualCsvIngestionTask`は**`data/manual_fallback/<provider>/<symbol>/<YYYYMMDD>/`配下の双子CSV**（`fallback_<provider>_<symbol>_<tf>_<YYYYMMDD>_op.csv`と`fallback_<provider>_<symbol>_<tf>_<YYYYMMDD>_review.csv`）を取り込み、`ManualCsvReconciler`が差分チェック・SHA256ハッシュ生成・承認者イニシャルの検証を行った後でのみ`bar_ready_queue`へ反映する。<ul><li>列順・必須列: `ts, open, high, low, close, volume, spread, session_tag`を既定とし、任意列は末尾に追加する。</li><li>型/単位: `ts`はUTCのISO8601（`YYYY-MM-DDTHH:MM:SSZ`）として受け取りUTCへ正規化後、レビュー用途にJSTへ換算したログを生成する。OHLC/`spread`は`Decimal`互換float、`volume`は非負float/整数、`session_tag`は`asia|london|newyork|overlap|holiday`を想定。</li><li>検証手順: 取り込み前に`tradectl data manual-template`で雛形を生成→`tradectl data validate-csv`でUTC→JST変換結果と5分足境界、単調増加、`low ≤ {open,close} ≤ high`、欠損・重複ゼロ、`volume`/`spread`非負、双子CSV間のSHA256一致を確認し、`ManualCsvReconciler`は不一致時にRunbook `RUN-DATA-06`の照合作業チェックリストを要求する。</li></ul>`metrics/data_ingestion_sla.jsonl`へ`phase=fetch|processing`別の遅延と429/403イベント（`event=rate_limited`, `backoff_sec`, `stage`)を記録し、30分内に4ペア全履歴を追いつけなければ`health.changed(reason=data_latency_fetch|process|rate_limited)`を発火する。`RateLimitGuard`は`config/sla_thresholds/<profile>.yaml`に定義した**トークンバケット閾値**（`tokens_per_minute`, `burst_tokens`, `poll_stages`)を読み込み、Stage0:平均15秒（`Uniform[12,15]`), Stage1:平均13秒（`Uniform[11,14]`), Stage2:平均11秒（`Uniform[10,12]`）の順に**ポーリング間隔を段階的に短縮**する。Stageごとのトークン残量・429発生率は`metrics/rate_limit_window.jsonl`へ書き出し、Validation Data Playbook（AC-45）と連携する。各StageでRolling 1hの429発生率≤1.0%かつ連続429<3回を7日連続で満たした場合のみ次Stageへ遷移し、429発生率>1.5%または連続429≥3回で直前Stageへロールバックする。トークン残量から`max_concurrent = ⌊tokens_per_minute / (60 / poll_interval_sec)⌋`を算定してフェッチスロット数を制御し、Stage0/1/2ではそれぞれ`min(max_concurrent, 4)`・`min(max_concurrent, 5)`・`min(max_concurrent, 6)`の上限で`ProviderFetchWorker`を有効化する。プロバイダp99遅延や連続失敗はRunbook `docs/runbooks/RUN-DATA-05.md`（フォールバック調整）/`docs/runbooks/RUN-DATA-06.md`（手動補填）と連携し、Acceptable Degradation期間中は`HealthState=degraded(fetch)`でSignal Boardへ`BoardMode=guarded`を指示して**新規提案を停止し、Reduce-Only運用と復旧チェックリストの進行のみを許可**する。データ鮮度検証→Reduce-Only運用→復旧確認→提案再開の完了シーケンスをRunbookチェックリストで証跡化し、`degraded_ack`監査イベントはこのシーケンス完了時のみ発行する。**公式API移行計画**として、M1.2でRefinitiv Elektron Lite / dxFeed FX / OANDA premium APIを比較し（費用≦月額¥80,000・ライセンスで個人利用許諾・REST/WebSocketで再配信不可を遵守）、PoCでp95≤12秒を満たしたプロバイダを`Data Ingestion Adapter`へ追加する。導入条件はコンプライアンス承認とSLA 30日ローリング合格、フェイルオーバー統合テスト（AC-45拡張）を満たすこと。 | yfinance, Dukascopy, CSV（M1）、有償API（M1.2+オプション） | M1 Core |
...
- **実装判断指針**:
  - **主経路優先順位**: ライブ取得はDukascopy→yfinance→ローカルキャッシュ→手動CSVの順とし、yfinanceはYahoo利用規約が想定する個人利用・非再配信の範囲に限定する。Dukascopy取得はHTTPバースト＋`ProviderFetchWorker`で**常時4ペア並列（必須ティッカー4種）**、Catch-up時は`max_fetch_workers=6`まで拡張する。取得した生データはプロバイダごとの`asyncio.Queue(maxsize>1)`に格納され、`ProviderParseWorker`が`AsyncBuffer`でUTC整列とQuality Guard検証を実施してから`bar_ready_queue`へ流す。Buffer飽和時は`priority=critical`の欠損補完ジョブが最優先で割り込む。
    - **CPUワーカー実行方針**: `ProviderParseWorker`とFeature Engine内のCPUバウンド処理は`asyncio.to_thread`（Python 3.11既定）で`ThreadPoolExecutor`へオフロードし、イベントループはI/O待ち専任とする。長期的に重い再計算が必要な場合は`ProcessPoolExecutor`へ差し替え可能なアブストラクションを維持する。`ExecutorFactory`で`max_workers = len(active_symbols) + 2`（主要4ペア＋予備2スロットを想定）を既定とし、Catch-up時のみ`+2`まで一時拡張する。
    - **バックプレッシャ制御**: `processing_queue_depth`が`max_workers * 2`を超えた場合はフェッチ側で`queue.put_nowait`例外を契機にポーリング間隔へ`backpressure_penalty_sec`（段階1: +2秒, 段階2: +5秒）を加算し、`AsyncBuffer`の未処理バー数に応じて`RateLimitGuard`のStage昇格を一時停止する。
    - **CPUワーカーメトリクス**: スレッドプールは`metrics/pipeline.jsonl`へ`{"metric":"worker","component":"ProviderParseWorker","processing_queue_depth":<int>,"worker_cpu_ms":<float>,"executor":"thread"}`形式で1分粒度の測定値を追記し、Catch-up/再計算フェーズでは`component`を`FeatureEngine`に切り替えて共通フォーマットで記録する。`metrics/pipeline_latency.jsonl`とは別にAC-05の確認対象として`worker_cpu_ms.p95`/`processing_queue_depth.p95`を週次レポートへ集計する。
...
- **プロバイダ別更新頻度/再接続ポリシー**: yfinanceは`config/sla_thresholds/<profile>.yaml::rate_limit.poll_stages`で定義したStage（0:平均15秒`Uniform[12,15]`, 1:平均13秒`Uniform[11,14]`, 2:平均11秒`Uniform[10,12]`）と`rate_limit.tokens_per_minute`を読み込み、`RateLimitGuard`のトークンバケット（`burst_tokens`含む）でポーリング間隔と同時実行数を制御する。Stage遷移要件は**429発生率≤1.0%（Rolling 1h）かつ連続429<3回を7日継続**で昇格、**429発生率>1.5%または連続429≥3回**で直前Stageへロールバックとし、ロールバック時は即座に`ProviderFetchWorker`のスロット上限を引き下げる。`RetryPolicy(max_attempts=3, backoff=1.5)`は各Stage共通で適用し、連続失敗時は`cooldown_sec=60`でリトライ抑制。DukascopyはHTTPバースト取得を**基準4秒周期（Stage連動で最大3秒まで短縮）**で行い、失敗時は`retry<=5`の指数バックオフ（最大間隔20秒）を実施する。WebSocketモード（M2+）は`ping_interval=15s`、`pong_timeout=5s`で監視し、`tradectl data reconnect --provider dukascopy`で強制リセット可能。手動CSVモードは`manual_source=true`で健康監視を除外するが、`tradectl data resume --provider <name>`で既定経路に戻す。
- **遅延伝播シーケンス**: `warning(fetch)`はEvent Bus経由で`Spread Monitor`へ配信し、対象シンボルの`SpreadCooldownState`に`recommended_action='halt'`を付与して通知する。OpsチームがRunbook `RUN-DATA-05`に従って`tradectl board --guarded --reason data_latency_fetch`を実行した場合にのみ`BoardGuardPolicy`がSignal Engineへ`SignalAction=block`をブロードキャストする。`warning(processing)`は`FeaturePipeline`の`bar_ready_queue`深さを計測して`AlertEvent`へ`recommended_action='runbook:RUN-DATA-05#processing_fallback'`を添付し、Reduce-Only運用へ切替えるかはオペレータ判断とする。`critical(fetch)`（`fetch_timeout`連続3回）は手動CSVフェイルオーバーとRunbook `docs/runbooks/RUN-DATA-05.md`の実施を要求する`AlertEvent`を生成し、Kill SwitchはOps/POが審査後に`tradectl kill-switch engage`を実行した場合のみ状態変更される。`critical(processing)`は`ManualCsvIngestionTask`完了まで`Workflow`をディレイキューに切替える推奨アクションを出力するが、既存ポジションのReduce-Only操作とチェックリスト更新は人手で継続可能とする。M2+では`Emergency Orchestrator`が`data_latency`プレイブックを評価し、CLI/メール通知と手動承認付きシナリオ起動を補助する。復旧後は`HealthMonitor.ack`コマンドで解除し、Spread Monitor・`bar_ready_queue`・`BoardMode`は手動解除ログに基づき通常状態へ戻す。
...
- **リスクフリーレート反映フロー**: 日本国債3か月物終値利回り（`risk_free.jp_tb_3m`）を`MarketRatesFetcher`が営業日毎（JPカレンダー）に取得し、UTC 07:00の定期ジョブで`data/market_rates/risk_free.parquet`へAppend保存する。ReporterはSharpe/Sortino算出前に評価対象ウィンドウと同じ営業日キーで`risk_free.parquet`を読み出し、`r_f_daily = (1+r_f_annual)^{1/252}-1`へ変換して日次リターン系列から控除する。欠損時は`risk_free_fallback.csv`（手入力テンプレート）をマージし、差分は`reports/validation_log/risk_free_patch_<date>.md`へ記録してValidation Data Playbookに連携する。
- **データバージョン固定**: AC-07向け検証は`data_manifest.json::m1_baseline_ma_rsi::2024-12-31`の`sha256`を基準とする。Reporterは週次実行時に`metrics.json::dataset_hash`を突合し、差分が検知された場合は`metric_state=pending(reason='dataset_hash_drift')`を付与して`reports/research/m1_baseline/validation_<date>.md`への差分記録とRunbook [STRAT-M1-VALIDATION](docs/runbooks/STRAT-M1-VALIDATION.md)の再承認フローを自動TODO化する。承認完了まではBenchmark Monitorも同じデータスナップショットを参照し、未承認データでは比較レポートを`status=draft`に留める。
...
- **Guardrailsメトリクス**: `metrics/guardrails.jsonl`は`schema/guardrails_metrics.schema.json`を正本とし、`risk_disclosure`と`auto_execute_forced_off`を追加してBoard/CLI/監査のキーを統一する。Runbook証跡は`reports/validation_log/AC-*/`へリンク。
...
- **Operations Readiness Loop**: Ops Readiness Evaluatorが`reports/governance/`, `reports/drill/`, `dist/offline_bundle/`のメタデータと`reports/governance/runbook_inventory_status.json`を収集し、バックアップ整合度・Runbook更新率・演習完遂率・緊急プロトコル検証の達成度を加重平均して`ops_readiness_score`を算出。Runbook項目は要件定義[§10.1 Runbookインベントリ](要件定義（テンプレ形式）v_1.md#runbook-inventory)の`status`と`review_due_in_days`を参照し、`status ∉ {ready, grace}`または`review_due_in_days < 0`でスコアを0に落とし`health.changed(reason=runbook_outdated)`を発火させる。スコア<75で新規リリース/戦略昇格コマンドをロックし、CLI `tradectl ops readiness --explain`でスコア構成と証跡ファイルを提示、証跡欠損はスコア0扱いとする（FR-63, NFR-28, AC-51）。Runbookレビュー完了（`status=ready`のみ）が運用準備の完了条件であり、リリースチェックリストにエクスポートしたステータスとレビュー議事録を添付する。
...
- イベント書き込み失敗時は最大3回リトライ（指数バックオフ1.5倍）。失敗後は`Critical`ログを出して`recommended_action=runbook:RUN-OPS-02#restore_event_bus`を付与し、OpsチームがRunbookに従って`hard_stop`移行の是非を手動判断する。
...
- **Health Monitor**: SPRT評価（FR-22）、Kill Switch関連メトリクス（FR-05）、連続エラー検出（FR-12）、SLAロールアップ（`SlaMinuteRollup`）を統合し、**計測と推奨アクションの提示に専念する**。`HealthState`は`{"status":"ok|soft_stop|hard_stop","reasons":[{"code":...,"detail":...,"recommended_action":...}]}`形式で保持し、SLA関連では`reasons[].code ∈ {data_latency_fetch, data_latency_processing, data_down, manual_stop, planned_maintenance, rate_limited}`を付与する。`health.changed`イベントには必ず計測根拠（`metric`, `observed_value`, `threshold`）とRunbook手順IDを含む`recommended_action`（例: `runbook:RUN-DATA-05#enter_guarded`, `notify:ops`, `sla_report:append`）を添付し、OpsチームはRunbook `RUN-DATA-05/06`に従って手動で`tradectl board --guarded`, `tradectl health manual-stop`, `tradectl kill-switch engage --reason <code>`等を実行するかを判断する。`HealthMonitor`自身は`board_mode`切替やKill Switch作動を行わず、CLIの`tradectl health show`/`tradectl health ack`で承認者とRunbook記録先を明示的に入力させる。
  - `degraded`状態が**連続3営業日**または**ローリング30日で2回**発生した場合は`health.escalate`イベントでレビュー会議を要求するが、会議結果を踏まえた`board_mode`変更やKill Switch決定はRunbookレビュー後にオペレータが手動承認する。`degraded`が**5営業日超継続**または週次レビュー2回連続で解消しない場合は`severity=critical`で通知し、Ops ManagerがRunbook `RUN-DATA-05`のKill Switch審査手順に沿って`tradectl kill-switch engage --reason <code>`を手動判断し、承認サインを`reports/validation_log/AC-45_sla_<date>.md`へ記録する。
  - `catch_up_lag_minutes`が警告閾値を超過した際は`notify:ops`や`pager:ingestion`などの推奨アクションと共に監査ログ必須項目（測定値・代替ソース実施状況・承認者）をRunbookへ記録するよう指示する。復旧時はRunbook手順に従い`catch_up_lag_minutes<30`、SLAメトリクス回復、手動CSV二重入力ハッシュ一致を確認した上でオペレータが`tradectl health resume`と`degraded_ack`監査イベントを発行する。

- **Board Guard Policy**: `HealthState.status`とは独立に`board_mode ∈ {normal, guarded, halted}`を追跡する。遷移はRunbook承認済みの手動コマンドのみで反映し、Opsチームは`tradectl board --guarded`/`tradectl board --normal`/`tradectl board --halted`実行時に承認者サインと根拠メトリクスを`docs/runbooks/RUN-DATA-05/06`へ記録する。Workflow Orchestratorは人間が実行したコマンド結果のみをSignal EngineとRisk Managerへ通知し、Signal Engineは通知された`board_mode`に応じて次バー評価前に`SignalAction`を切り替えるが、自身からモード変更を要求しない。Reduce-Only運用中は`allow_reduce_only=true`フラグを添付した制御メッセージを用いて既存候補をフィルタし、解除には「データ鮮度検証→Reduce-Only運用→復旧確認→提案再開」のRunbookチェックリスト完了と`degraded_ack`監査イベント記録が必要となる。`halted`はRunbook `RUN-RISK-01`でKill Switchを手動実行した場合にのみ設定する。状態遷移と運用アクションは以下。

| 現状態 | トリガー | 次状態 | オペレータ対応 |
| --- | --- | --- | --- |
| ok | Runbook `RUN-RISK-01`レビューでKill Switch停止が妥当と判断 | soft_stop | Ops/POが`tradectl kill-switch engage --reason <code>`を手動実行し、`tradectl status`で原因を共有。解除時はRunbookチェック完了後に`tradectl kill-switch release --ticket <id>`→`tradectl health resume`を順に実行。
| ok | Spread guardチェックリストで閾値超過をRunbook承認 | soft_stop | `tradectl health manual-stop --reason spread_guard`等で停止し、`tradectl spread cooldown`で監視しつつRunbook `RUN-RISK-02`に従ってReduce-Only運用を継続。解除は責任者承認後に`tradectl health resume`を実行。
| ok | Runbook `RUN-DATA-05`でデータ欠損対応が必要と承認 | soft_stop | `tradectl health manual-stop --reason data_gap`で停止し、`tradectl resync`/手動CSV投入を手順通りに実施。正常バー3本確認と承認サイン取得後に`tradectl health resume`で復帰。
| ok | SLA遅延レビューでGuarded運用が必要と承認 | board_guarded | Opsが`tradectl board --guarded --reason <code>`を実行し、Workflow Orchestrator経由でSignal Engineが`SignalAction=block`へ切替え。Runbook `RUN-DATA-05/06`に沿って代替ソース運用とチェックリストを完了させ、SLA回復＋PO/Opsダブルサイン＋`degraded_ack`発行後に`tradectl board --normal`を実行。
| ok | processing遅延/バッチ詰まりで手動介入が承認 | board_guarded | `tradectl board --guarded`でReduce-Onlyモードへ切替え、`ManualCsvIngestionTask`や再処理をRunbook手順通りに実施。`reports/validation_log/AC-45_sla_<date>.md`に対応メモとレビュー結果を残し、`bar_ready_queue`安定後に`tradectl board --normal`を実行。
| soft_stop | Runbook完了後に復帰承認済み | ok | 承認サインが揃い次第`tradectl health resume`。Kill Switch解除時は`docs/runbooks/RUN-RISK-01.md`のチェックリストどおりに実施。
| 任意 | `tradectl stop`/manual emergency、クリティカル例外をRunbookで承認 | hard_stop | プロセス停止。再起動前に原因調査し、Runbook `RUN-OPS-01`で承認後に`tradectl start`を実行。
| hard_stop | 再起動後に整合性確認完了 | ok | スナップショットから復旧後、`tradectl status`で整合性確認しRunbookサインを取得。
...
- **RetryPolicy**: インフラ層でユニット化し、yfinance/Dukascopy取得は`max_attempts=3`, `backoff=1.5`倍増。取得失敗は`DataDegraded`イベントで通知し、閾値超過時は`AlertEvent`へ`recommended_action='runbook:RUN-DATA-05#kill_switch_review'`を添付してKill Switch審査を促す。CLIコマンドは即時エラーとし、非ゼロ終了コードでユーザーへ通知。
- **Emergency Orchestrator**〈M2+〉: `emergency.yaml`を`pydantic`でロードし、シナリオ→アクション（Kill Switch遷移、Reduce-Only提案、通知、再接続リトライ）を状態マシンとして実行（FR-47, AC-38）。`tradectl emergency dry-run`で手順検証、`tradectl emergency trigger <scenario>`で手動発火（保護付き）。M1 Coreでは`EmergencyOrchestratorStub`が`noop`ログのみを残し、本体は未導入のまま運用する。緊急時のオペレーション責務と承認テンプレートはRunbook `RUN-EMER-UNWIND-01`（Kill Switch STOPポジション・アンワインド）と`RUN-DATA-05`を参照し、CLI `tradectl emergency close-all/cancel-open/hedge`をスタブのまま演習で確認する。
...
26. Audit Bundle Serviceが月次/四半期スケジュールまたは`tradectl audit bundle --period`コマンドに応じて、シグナル履歴・承認/約定ログ・設定差分・リスク承諾を`audit_pack/<period>/`へ束ね、`audit_manifest.json`と署名`audit_manifest.sig`を生成（FR-59）。〔M1.1〕[^ms-hardening]
27. Release Governance Serviceが`tradectl release prepare/tag`でSmokeテスト結果とリスク承諾差分を検証し、未完了チェック項目があれば`auto_execute=false`へ切替して新規配信を抑止。承認結果は`reports/audit/release/<version>.md`に記録（FR-60）。〔M1.1〕[^ms-hardening]
...
- **`tradectl board`**: Signal Board表示コマンド。入力として`logs/events/<YYYYMMDD>.jsonl`を日付降順で探索し、最新の日次イベントファイルをストリームして最新バーごとに表形式レンダリング。出力列は`symbol, side, entry, size, sl, tp, score, ttl, badges`。`--filter symbol=USDJPY`や`--view open_tickets`などのフィルタ/ビュー切替を提供。起動時には`RiskDisclosureService`で承諾ステータスを確認し、**M1 Core**では未承諾/期限切れ時にヘッダへリスク警告バナーと承諾誘導リンクを表示して読み取り専用モード（承認/却下は警告付きで実行可）へ切り替える。**M1.1以降**は初回起動および四半期レビュー週の初回起動時にリスク警告ダイアログ（投資助言禁止・主要リスク・損失可能性・想定利用範囲・ブローカー約款リンク・直近承諾ログサマリを含む）を必須表示し、承諾完了までボード描画と高リスクCLIをロックする。承諾操作完了後は`audit`イベント（type=`risk_consent`）へ承諾文言・表示バージョン・ユーザーID・端末識別子・コンセントハッシュを記録してからレンダリングへ遷移する。 Acceptable Degradation（`board_mode=guarded`）時はヘッダに橙色バナーを表示し、新規提案セクションを強制的にロックしてReduce-OnlyキューとRunbookチェックリスト進行ログのみを表示する。Reduce-Only操作実行時にはダブルチェック済みトグルと補完ソース（Dukascopy/yfinance/manual_fallback）を必須入力とし、チェックリスト完了時に`audit`へ`degraded_ack`イベントを追記する。`board_mode=guarded`解除後はバナーが自動で消え、解除ログと完了チェックリストリンクを履歴に残す。
...
- **高リスク操作の警告ガード**: `tradectl ticket approve`（ライブモード、R>既定閾値）、`tradectl emergency trigger`, `tradectl reduce-only push`等の高リスクコマンドは実行前に`RiskDisclosureService`へ承諾ステータスを照会する。**M1 Core**では未承諾・期限切れ・文言更新待ち・端末変更検知の場合に`[WARN] Risk disclosure consent pending`メッセージとRunbook参照リンクを表示し、利用者判断で続行できる。**M1.1以降**は同条件で承諾ダイアログを強制表示して承諾完了までコマンド実行をブロックし、承諾更新時に`audit`イベント（`risk_consent`)を追記、コマンド側の`audit`イベント（`ticket_action`, `emergency_action`など）へ`consent_reference_id`と`consent_version_hash`フィールドを紐付けて監査可能性を担保する。
...
- **`tradectl status`**: セッションのヘルスと統計を表示。`HealthState`（`status`, `reasons`）と`Snapshot`のハッシュ、現在のSpreadCooldownStateや未処理Reduce-Onlyチケット数、`benchmark_gap_pct`、直近ジャーナルハイライト（最新コメント/評価）を含む。`--detail`指定時はJSONセクションを追加し、`board_guard.board_mode`/`board_guard.reduce_only`/`board_guard.guard_reason`/`board_guard.checklist_state`でSignal Boardガード状況を、`data_sources.manual_source`/`data_sources.active_provider`でデータ経路の切替状態を照会できるようにする。
...
- **`tradectl audit bundle --period <YYYYMM>`**: `audit_pack/<period>/`配下にシグナル履歴・承認/約定ログ・設定差分・リスク承諾ログを集約し、`audit_manifest.json`と署名`audit_manifest.sig`を生成。ベンチマーク比較はM2+の拡張対象。`--verify`で署名検証を実施し、検証結果は`reports/audit/audit_pack/<period>.md`に記録。
...
- **リスク開示と`audit`イベント連携**: `RiskDisclosureService`は承諾バージョンと期限を`consent_state.json`に保存し、承諾/拒否/期限切れイベントを`audit`イベントストアへ`risk_consent`タイプで記録する。**M1 Core**では承諾未取得時に`consent_state.status=pending`として記録しつつSignal Boardや高リスクコマンドへ警告ステータスを返却し、操作イベントには`consent_reference_id`を空にしたまま`consent_warning=true`フラグを残す。**M1.1以降**は承諾未取得時にCLIを終了コード`103`でブロックし、承諾ダイアログ通過後の操作イベントへ最新`risk_consent`イベントIDと`consent_version_hash`を付与して週次ガバナンスレポートと監査エクスポートでトレースできるようにする。
...
- **シグナル評価**: 再計算したバーについてRegime/Scoring/Execution/Risk/Sizingを再実行し、TTL切れやドリフト超過のチケットは自動失効マーク。Acceptable Degradation中は`BoardMode=guarded`で新規提案を停止し、Reduce-Only運用と復旧チェックリスト完了後に`ResyncCompleted`イベントで復旧レポートを生成、OpsチームがRunbook確認のうえ`tradectl board --normal`を手動実行して通常モードへ戻す。
...
## 7. 非機能要件への対応方針
- **性能**: pandasベース処理＋numba/ポリシー最適化と差分再計算キャッシュで5分足ストリーム遅延<100msを目標化。I/Oは非同期キューで平滑化し、CPUバウンド区間は`asyncio.to_thread`経由のスレッドプールに限定してオフロードする。`perf_counter`計測を`on_bar_in`～`board_render`で常時実施し、p95/p99を`metrics/pipeline.jsonl`へ記録して`tradectl metrics report`で集計しNFR-01/AC-05を監視する。加えて、`worker_cpu_ms.p95>450`または`processing_queue_depth.p95>2*(len(active_symbols)+2)`を検出した場合は`HealthMonitor.raise(level='warning', reason='cpu_worker_slow')`でアラートし、`ExecutorFactory.scale(+2)`（上限`len(active_symbols)+4`）または`FeatureEngine`の遅延再キュー（`queue.defer(oldest)`）を実施して回復を図る。閾値超過が連続3ロールアップで継続した場合は`BoardMode=guarded`へ遷移させ、CPUワーカーメトリクスをAC-05測定ロジックで記録しつつスレッドプール縮退後の再評価（Queue深さp95が既定値未満）を解除条件とする。
...
- **プレフライト検査（推奨）**: `tradectl preflight`が起動前に`systemsetup -getnetworktimeserver`/`sntp -sS $(config.time.ntp_server)`を実行し、NTP到達性と偏差を取得。`abs(clock_drift_ms)>500`またはNTPサーバ未設定時はWARNログとRunbook `RUN-TIME-01`参照メッセージを出力し、`HealthState`は維持する。オペレータが必要に応じて`board_mode=guarded`を選択する。
- **ランタイム監視**: `TimeSyncGuard`が10分間隔で`python -c "import time;print(time.time())"`と`ntpdate -q`（macOSでは`/usr/sbin/sntp -sS`のdry-run）を比較し、結果を`metrics/time_sync.jsonl`へ`{"ts_utc", "clock_drift_ms", "ntp_server", "mode"}`として追記。M1 Coreではドリフト>1500ms時にWARNログと`health.suggest_guarded(clock_out_of_sync)`イベントを発行するのみで自動切替は行わず、強制ガードはM1.1 Hardeningの対象とする。
- **UTC固定のデータ書き込み**: Data Ingestion/Manual CSV/バックテスト出力は全て`datetime64[ns, UTC]`で保存し、CSV入力では`utc_iso`列の存在とタイムゾーン付きISO8601をschema検証。`data/manual_fallback/.../_review.csv`側もUTCで入力し、`ManualCsvReconciler`がJST等へのずれを検出した場合は`clock_mismatch`エラーで取込拒否する。
- **ローカルタイムとの相互運用**: CLI表示はユーザー設定の`config/profile.yaml:display_timezone`で変換し、監査ログとParquetはUTC保持。Runbook `docs/runbooks/RUN-TIME-01.md`は日次起動前の手動時計差確認と、必要時の`sudo sntp -sS time.apple.com`→`tradectl preflight --recheck`→結果記録の手順を定義する。
...
| --- | --- | --- | --- | --- |
| データ品質監査 | 平日毎日 | `data/manual_fallback`二重入力レビュー、`health.changed`承認、`reports/validation_log`更新 | 35〜45分/日 | `tradectl data manual-report --template`でCSVチェックリスト自動生成、Slack/メール通知のドラフト化 |
...
| Runbook整備レビュー | `make check-runbooks`のCI結果が`reports/governance/runbook_inventory_status.json`で全項目`status=ready`または`grace`（猶予<7日）となり、`release_checklist`にRunbookレビュー議事録リンクを添付 | §9 運用・保守, [§10.1 Runbookインベントリ](要件定義（テンプレ形式）v_1.md#runbook-inventory) | AC-43, AC-45 |
