...
def test_build_daily_shadow_ops_summary_surfaces_multi_pair_preparation_result(tmp_path: Path) -> None:
    ...
    assert ops_summary["multi_pair_expansion_gate_status"] == "blocked"
...
def test_build_daily_shadow_ops_summary_advances_when_post_qualification_consistent(
    ...
    assert ops_summary["multi_pair_next_review_bridge_status"] == "re_review_required"
    assert ops_summary["headline"] == "blocked: re_review_next_pair_review_handoff"
    assert ops_summary["next_action"] == "re_review_next_pair_review_handoff"


def test_build_daily_shadow_ops_summary_advances_next_review_bridge_to_expansion_start(
    tmp_path: Path, monkeypatch
) -> None:
    summary = _summary()
    summary["posture"] = "stable_shadow"
    ...
    summary["multi_pair_expansion_current_symbol"] = "GBPUSD"
    summary["multi_pair_expansion_next_symbol"] = "AUDUSD"
    summary["multi_pair_next_expansion_current_symbol"] = "EURJPY"
    summary["multi_pair_next_expansion_next_symbol"] = "AUDUSD"
    summary["multi_pair_next_expansion_status"] = "ready_to_start"
    summary["multi_pair_next_expansion_execution_status"] = "missing"
    ...
    monkeypatch.setattr(
        ...
        lambda _ops_summary: {
            ...
            "promotion_status": "ready_for_next_pair_review",
            "recommended_action": "review_next_pair_candidate",
            "active_symbols": ["USDJPY", "EURUSD", "GBPUSD", "EURJPY"],
            "current_symbol": "GBPUSD",
            "expanded_symbol": "EURJPY",
            "next_symbol": "AUDUSD",
            "next_pair_metadata": {},
    ...
    monkeypatch.setattr(
        "src.interfaces.gui.shadow_daily_ops.build_multi_pair_next_expansion_execution_summary",
        lambda _ops_summary, ledger_path=None: {
            "status": "ready_to_start",
            "execution_status": "missing",
            "current_symbol": "EURJPY",
            "next_symbol": "AUDUSD",
    ...
    ops_summary = build_daily_shadow_ops_summary(
        summary,
        multi_pair_next_review_bridge_history_path=tmp_path / "multi_pair_next_review_bridge_history.jsonl",
    )

    assert ops_summary["multi_pair_next_review_bridge_status"] == "ready_for_review_start"
...
def test_build_daily_shadow_ops_summary_surfaces_pair_expansion_gate_when_pilot_qualified(tmp_path: Path) -> None:
    ...
    assert ops_summary["multi_pair_pilot_completion_gate_status"] == "qualified_for_pair_expansion"
    assert ops_summary["multi_pair_expansion_gate_status"] == "ready_for_pair_expansion"
    assert ops_summary["multi_pair_expansion_current_symbol"] == "EURUSD"
    assert ops_summary["multi_pair_expansion_next_symbol"] == "GBPUSD"
    assert ops_summary["multi_pair_expansion_rollout_status"] == "missing"
    assert ops_summary["headline"] == "ready: start_pair_expansion_rollout"
    assert "multi_pair_preparation --next-symbol GBPUSD" in ops_summary["multi_pair_expansion_runner_command"]
    assert "Multi-Pair Pair Expansion Gate" in render_daily_shadow_ops_report(ops_summary)


def test_build_daily_shadow_ops_summary_surfaces_pair_expansion_rollout_evidence(tmp_path: Path) -> None:
    summary = _summary()
    summary["generated_at_utc"] = "2026-03-21T09:00:00Z"
    summary["review_date_utc"] = "2026-03-21"
    summary["stage_gate_summary"] = {"status": "ready", "ready_for_next_stage": True, "next_action": "candidate_onboarding_or_multi_pair_preparation_ready", "reasons": []}
    summary["soak_summary"] = {
        "status": "qualified",
        "ready_for_transition": True,
        "qualified_next_phase": "multi_pair_preparation",
    ...
    baseline_validation.write_text(
        json.dumps({"results": [{"window_name": "2016_2025", "summary": {"pf": 1.2, "avg_r": 0.03, "max_drawdown": 0.1}, "acceptance": {"status": "pass"}}, {"window_name": "2022_2025", "summary": {"pf": 1.25, "avg_r": 0.05, "max_drawdown": 0.08}, "acceptance": {"status": "pass"}}]}) + "\n",
        encoding="utf-8",
    )
    validation.write_text(
        json.dumps({"results": [{"window_name": "2016_2025", "summary": {"pf": 1.23, "avg_r": 0.032, "max_drawdown": 0.11}, "acceptance": {"status": "pass"}}, {"window_name": "2022_2025", "summary": {"pf": 1.29, "avg_r": 0.052, "max_drawdown": 0.085}, "acceptance": {"status": "pass"}}]}) + "\n",
        encoding="utf-8",
    )
