...
def test_poc_result_can_export_returns_and_equity(tmp_path: Path) -> None:
    ...
    assert (
        returns_path.with_suffix(".parquet").exists()
        or returns_path.with_suffix(".csv").exists()
        or returns_path.exists()
    )
    assert (
        equity_path.with_suffix(".parquet").exists()
        or equity_path.with_suffix(".csv").exists()
        or equity_path.exists()
    )
