...
def _bar(ts: datetime, *, o: float, h: float, l: float, c: float, v: float = 1.0) -> dict[str, object]:
    return {
...
def test_quality_guard_accepts_clean_bars() -> None:
    ...
    bars = [_bar(start + timedelta(minutes=5 * i), o=1, h=2, l=0.5, c=1.5) for i in range(5)]
...
def test_quality_guard_flags_ohlc_bounds() -> None:
    ...
    bars = [_bar(ts, o=2.0, h=1.0, l=0.5, c=2.5)]
    frame = MarketFrame(symbol="USDJPY", timeframe="5m", bars=bars)
...
def test_quality_guard_flags_gap() -> None:
    ...
    start = datetime(2025, 1, 1, 0, 0, tzinfo=timezone.utc)
    bars = [
        _bar(start, o=1, h=2, l=0.5, c=1.2),
        _bar(start + timedelta(minutes=30), o=1, h=2, l=0.5, c=1.2),
    ]
    frame = MarketFrame(symbol="USDJPY", timeframe="5m", bars=bars)
