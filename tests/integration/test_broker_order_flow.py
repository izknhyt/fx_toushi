from __future__ import annotations

from pathlib import Path

from src.brokers.order_lifecycle import OrderLifecycleManager
from src.brokers.order_store import OrderStateStore
from src.brokers.stage_guard import AutonomyStageGuard


def test_broker_order_flow(tmp_path: Path) -> None:
    store = OrderStateStore(root_dir=tmp_path)
    guard = AutonomyStageGuard(stage="reduce_only", state_path=tmp_path / "stage.json")
    lifecycle = OrderLifecycleManager(store=store, stage_guard=guard)

    envelope = lifecycle.create(
        {
            "ticket_id": "ticket-1",
            "mode": "paper",
            "profile": "paper",
            "strategy_id": "strat-1",
            "reduce_only": True,
        },
        stage_guard_ctx={"stage": "reduce_only"},
    )
    lifecycle.update_state(envelope.order_id, status="pending_ack", mode="paper")
    lifecycle.attach_fill(
        envelope.order_id,
        mode="paper",
        fill_event={
            "ticket_id": "ticket-1",
            "order_id": envelope.order_id,
            "adapter": "sandbox",
            "profile": "paper",
            "fill_price": 1.2345,
        },
    )
    receipt = lifecycle.finalize(envelope.order_id, mode="paper")
    assert receipt.status == "reconciled"
