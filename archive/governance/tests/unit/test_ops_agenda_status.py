...
def test_agenda_surfaces_next_review_bridge_start(tmp_path: Path) -> None:
    health_state_path = tmp_path / "snapshots" / "latest" / "health_state.json"
    _write_health_state(health_state_path, [], status="ok")
        ...
        json.dumps(
            {
                ...
                "review_date_utc": "2026-03-24",
                "headline": "ready: start_next_pair_expansion_rollout",
                "alert_level": "none",
                "recommended_action": "start_next_pair_expansion_rollout",
                "multi_pair_post_qualification_status": "consistent",
                "multi_pair_next_review_bridge_status": "ready_for_review_start",
                "multi_pair_next_review_bridge_recommended_action": "start_next_pair_expansion_rollout",
                "multi_pair_next_review_bridge_stable_streak_days": 2,
    ...
    matching = [task for task in ctx.operational_tasks if task["task"] == "Start next pair expansion rollout"]
    assert len(matching) == 1
    notes = str(matching[0]["notes"])
    assert "next_review_bridge=ready_for_review_start:start_next_pair_expansion_rollout" in notes
    assert "next_review_bridge_expanded=EURJPY" in notes
    assert "next_review_bridge_next=AUDUSD" in notes


def test_agenda_surfaces_v2_completion_candidate(tmp_path: Path) -> None:
    health_state_path = tmp_path / "snapshots" / "latest" / "health_state.json"
    _write_health_state(health_state_path, [], status="ok")
        ...
        json.dumps(
            {
                ...
                "multi_pair_cycle_recommended_action": "review_next_pair_candidate",
                "multi_pair_cycle_qualified_streak_days": 3,
                "v2_completion_status": "complete_candidate",
                "v2_completion_recommended_action": "record_v2_completion_evidence",
    ...
    matching = [task for task in ctx.operational_tasks if task["task"] == "Review v2 completion evidence"]
    assert len(matching) == 1
    notes = str(matching[0]["notes"])
    assert "v2_completion=complete_candidate:record_v2_completion_evidence" in notes
    assert "v2_completion_candidate=true" in notes
...
def test_agenda_creates_next_pair_expansion_rollout_task_after_next_pair_review(tmp_path: Path) -> None:
    notification_log = tmp_path / "logs" / "ops" / "shadow_daily_notifications.jsonl"
    notification_log.parent.mkdir(parents=True, exist_ok=True)
    notification_log.write_text(
        json.dumps(
            {
                ...
                "multi_pair_next_expansion_recommended_action": "start_next_pair_expansion_rollout",
            }
        )
        + "\n",
        encoding="utf-8",
...
def test_agenda_reviews_following_pair_after_next_expansion_steady_state(tmp_path: Path) -> None:
    notification_log = tmp_path / "logs" / "ops" / "shadow_daily_notifications.jsonl"
    notification_log.parent.mkdir(parents=True, exist_ok=True)
    notification_log.write_text(
        json.dumps(
            {
                ...
                "multi_pair_steady_state_status": "ready_for_next_pair_review",
                "multi_pair_steady_state_recommended_action": "review_next_pair_candidate",
                "multi_pair_steady_state_next_symbol": "AUDUSD",
            }
        )
        + "\n",
        encoding="utf-8",
    ...
    assert matching
    assert "pair_steady_state=ready_for_next_pair_review:review_next_pair_candidate" in str(matching[0]["notes"])
    assert "pair_steady_state_next=AUDUSD" in str(matching[0]["notes"])
