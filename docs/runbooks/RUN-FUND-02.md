...
4. CLI `tradectl funding sync --shadow reports/funding/swap_rates_shadow.csv`を再実行し、イニシャルとハッシュ値を記録する。同期が成功したら、CLI出力と`funding_state.json`を`docs/implementation_packets/<packet>/evidence/`に保存する。
