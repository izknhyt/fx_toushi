...
- 参照セクション: detailed_design_fx_signal_tool_v1.md §3.0, §3.1.1
 - 依頼Issue/PR: docs/change_requests/20250318_packet_backlog.md#pkg-data-status-01
- 作成日: 2025-03-15
- 作成者: Codex Liaison
...
| --- | --- | --- | --- |
| tests/cli/test_data_status_cli.py | `pytest.mark.data_status_cli`でstage_eval記録を検証。 | `pytest -k "data_status_cli"` | N/A |
...
## 3. チェックリスト
- [x] 設計整合: detailed_design_fx_signal_tool_v1.md §3.0, §3.1.1 を確認
- [x] テスト実行: `poetry run pytest -k "data_status_cli"`
- [x] 監査ログ検証: `metrics/rate_limit_window.jsonl` 最新行に`stage_eval`が含まれることを確認
- [x] Rollback手順記載: docs/runbooks/RUN-DATA-05.mdへテスト失敗時のFallbackを追記
- [x] Trader Sign-offテンプレ発行: docs/trader_signoff/PKG-DATA-STATUS-01.md

## 4. エビデンス
- CLI/スクリーンショット: docs/trader_signoff/PKG-DATA-STATUS-01.md（`tradectl data status --log-stage-eval`出力を貼付）
- メトリクス: reports/implementation/20250315_pkg-data-status-01/metrics/
- ログ: reports/implementation/20250315_pkg-data-status-01/logs/data_status_watch.log
