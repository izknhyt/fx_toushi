"""US Opening Range Breakout + VWAP retest continuation strategy."""
...
class _SessionState:
    ...
    retest_deadline: datetime | None = None
    ...
    def reset_breakout(self) -> None:
        ...
        self.retest_deadline = None


class UsOpeningRangeBreakoutVwapRetestStrategy(StrategyPluginProtocol):
    """Detects US ORB and enters only after VWAP-aligned retest continuation."""

    id = "m1_us_orb_vwap_retest"
    determinism_key = "us_orb_vwap_retest_v1"
    metadata = StrategyMetadata(
        name="M1 US ORB + VWAP Retest Continuation",
        version="0.1.0",
        ...
        tags=frozenset({"us_session", "orb", "vwap", "retest", "breakout"}),
    )
    ...
    @staticmethod
    def _parse_session_range(value: object) -> tuple[int, int]:
    ...
    def generate_signals(self, context: StrategyContext) -> Iterable[OrbVwapRetestSignal]:
        ...
        session_start, session_end = self._parse_session_range(
            entry.get("session_utc_range", filters.get("session_utc_range", "13-21"))
        )
        session_start_hour_default = session_start
        session_start_token = entry.get("session_start_utc")
        if session_start_token is None:
            session_start_token = "13:30" if session_start == 13 else f"{session_start:02d}:00"
        start_hour, start_minute = self._parse_hhmm(
            session_start_token, session_start_hour_default
        )
        ...
        orb_minutes = max(
            timeframe_minutes,
            self._coerce_int(entry.get("orb_minutes"), 30),
        )
        retest_max_bars = max(1, self._coerce_int(entry.get("retest_max_bars"), 6))
        ...
        if not allowed_session_tags:
            allowed_session_tags = frozenset({"newyork", "overlap", "us"})
        ...
        min_breakout_abs = max(0.0, self._coerce_float(filters.get("min_breakout_abs"), 0.03))
        breakout_atr_mult = max(0.0, self._coerce_float(filters.get("breakout_atr_mult"), 0.25))
        breakout_cost_mult = max(0.0, self._coerce_float(filters.get("breakout_cost_mult"), 3.0))
        vwap_bias_cost_mult = max(0.0, self._coerce_float(filters.get("vwap_bias_cost_mult"), 0.5))
        min_retest_abs = max(0.0, self._coerce_float(filters.get("retest_min_abs"), 0.01))
        retest_cost_mult = max(0.0, self._coerce_float(filters.get("retest_cost_mult"), 1.2))
        retest_atr_mult = max(0.0, self._coerce_float(filters.get("retest_atr_mult"), 0.05))
        ...
        emitted: list[OrbVwapRetestSignal] = []
        for symbol in symbols:
            ...
            if volume_value > 0:
                state.vwap_pv += close_value * volume_value
                state.vwap_volume += volume_value
            vwap_value = (state.vwap_pv / state.vwap_volume) if state.vwap_volume > 0 else None

            if now < state.orb_end:
                state.orb_bars += 1
                state.orb_high = high_value if state.orb_high is None else max(state.orb_high, high_value)
                state.orb_low = low_value if state.orb_low is None else min(state.orb_low, low_value)
                continue

            min_orb_bars = max(1, orb_minutes // timeframe_minutes)
            if state.orb_bars < min_orb_bars or state.orb_high is None or state.orb_low is None:
                continue

            orb_width = max(0.0, state.orb_high - state.orb_low)
            if max_cost_to_range_ratio >= 0 and orb_width > 0:
                if (cost_estimate / orb_width) > max_cost_to_range_ratio:
                    continue
            elif max_cost_to_range_ratio >= 0 and orb_width <= 0:
                continue
            ...
            vwap_bias = cost_estimate * vwap_bias_cost_mult
            ...
            if state.breakout_direction is None:
                ...
                long_breakout = (
                    ...
                    and close_value > (state.orb_high + breakout_buffer)
                    and close_value >= (vwap_value + vwap_bias)
                )
                short_breakout = (
                    "short" in allowed_directions
                    and close_value < (state.orb_low - breakout_buffer)
                    and close_value <= (vwap_value - vwap_bias)
                )
                if not long_breakout and not short_breakout:
                    continue
                state.breakout_direction = "long" if long_breakout else "short"
                state.breakout_level = state.orb_high if long_breakout else state.orb_low
                state.breakout_price = close_value
                state.breakout_at = now
                state.retest_deadline = now + timedelta(minutes=timeframe_minutes * retest_max_bars)
                continue

            if state.retest_deadline and now > state.retest_deadline:
                state.reset_breakout()
                continue
            if vwap_value is None:
                continue

            direction = state.breakout_direction
            if direction is None or state.breakout_level is None or state.breakout_price is None:
                state.reset_breakout()
                continue
            if direction == "long" and close_value < state.orb_low:
                state.reset_breakout()
                continue
            if direction == "short" and close_value > state.orb_high:
                state.reset_breakout()
                continue

            retest_tolerance = max(
                min_retest_abs,
                cost_estimate * retest_cost_mult,
                atr_value * retest_atr_mult,
            )
            if direction == "long":
                touched_vwap = low_value <= (vwap_value + retest_tolerance)
                held_direction = close_value >= vwap_value and close_value >= state.breakout_level
                continuation = close_value >= state.breakout_price and close_value >= open_value
                breakout_distance = close_value - state.breakout_level
            else:
                touched_vwap = high_value >= (vwap_value - retest_tolerance)
                held_direction = close_value <= vwap_value and close_value <= state.breakout_level
                continuation = close_value <= state.breakout_price and close_value <= open_value
                breakout_distance = state.breakout_level - close_value

            if not (touched_vwap and held_direction and continuation):
                continue
            ...
            emitted.append(
                OrbVwapRetestSignal(
                    ...
                    rationale="us_orb_breakout_vwap_retest_continue",
                    score=round(quality, 4),
                    quality_score=round(quality, 4),
                    orb_high=round(state.orb_high, 6) if state.orb_high is not None else None,
                    orb_low=round(state.orb_low, 6) if state.orb_low is not None else None,
                    orb_width=round(orb_width, 6),
                    vwap=round(vwap_value, 6) if vwap_value is not None else None,
    ...
    def evaluate(self, context: StrategyContext) -> Iterable[OrbVwapRetestSignal]:
        return self.generate_signals(context)


__all__ = ["UsOpeningRangeBreakoutVwapRetestStrategy", "OrbVwapRetestSignal"]
